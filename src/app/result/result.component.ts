...
export class ResultComponent implements OnInit {
  ...
  inputvail() {
    ...
    this.vail = this.num.length >= 3&&result.length==10 ? 'is-valid' : 'is-invalid';
  }

  search() {
    this.result = [];
    this.lottery_num=this.num;
    var _data = this.select_value=='all'?this.quontupleList:this.quontupleList.filter(data => data.week == this.select_value);
    _data.forEach(data => {
